...
public class AlarmRestartService extends WakeIntentService {
    ...
    void doReminderWork(Intent intent) {
        ...
        while (!cursor.isAfterLast()) {
            Log.d(
                    "DEBUG1",
                    "Check Restarting alarm for time " +
                            cursor.getString(cursor.getColumnIndex(DBHelper.TODO_DATETIME))
            );
            try {
                if (!new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.SSSZ")
                        .parse(cursor.getString(cursor.getColumnIndex(DBHelper.TODO_DATETIME)))
                        .before(new Date())) {
                    Intent myIntent = new Intent(AlarmRestartService.this, OnAlarmReceiver.class);
                    myIntent.putExtra(
                            DBHelper.TODO_DESC,
                            cursor.getString(cursor.getColumnIndex(DBHelper.TODO_DESC))
                    );
                    sp = getSharedPreferences(myPreference, Context.MODE_PRIVATE);
                    pendingId = sp.getInt("pendingId", 0);
                    PendingIntent pendingIntent = PendingIntent.getBroadcast(
                            AlarmRestartService.this,
                            pendingId,
                            myIntent,
                    ...
                    Log.d(
                            "DEBUG1_DATE",
                            "date obtained from db: " + cursor.getString(
                                    cursor.getColumnIndex(DBHelper.TODO_DATETIME)) +
                                    " DESC: " + cursor.getString(
                                    cursor.getColumnIndex(DBHelper.TODO_DESC)
                            )
                    );
                    ...
                    String s = "";
                    String s1 = "";
                    date = simpleFormat.parse(cursor.getString(
                            cursor.getColumnIndex(DBHelper.TODO_DATETIME)
                            )
                    );
